...
class MyMetaverseFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
         ...
         * OBJECTS Platform
         */
        $p1 = new Platform(1, "Nintendo Switch");
        $manager->persist($p1);
        $p2 = new Platform(2, "PC Steam");
        $manager->persist($p2);  
        $p3 = new Platform(3, "Playstation 5");
        $manager->persist($p3);
        $p4 = new Platform(4, "GameBoy Advanced");
        $manager->persist($p4);
        $p5 = new Platform(5, "GameBoy / GameBoy Color");
        $manager->persist($p5);
        $p6 = new Platform(6, "Playstation 4");
        $manager->persist($p6);
        $p7 = new Platform(7, "Nintendo DS");
        $manager->persist($p7);
        $p8 = new Platform(8, "Nintendo 3DS");
        $manager->persist($p8);
        $p9 = new Platform(9, "PlayStation 3");
        $manager->persist($p9);
        $p10 = new Platform(10, "PlayStation 2");
        $manager->persist($p10);
        $p11 = new Platform(11, "PlayStation");
        $manager->persist($p11);
        $p12 = new Platform(12, "XBox");
        $manager->persist($p12);
        $p13 = new Platform(13, "XBox 360");
        $manager->persist($p13);
        $p14 = new Platform(14, "XBox One");
        $manager->persist($p14);
        $p15 = new Platform(15, "XBox Series");
        $manager->persist($p15);
        ...
        $u2 = new User("Dupont", "Arthur", "arthur-dupont@hotmail.fr", "arthurdu92", 
            "", "Homme", new DateTime("1999-07-14"), false, 
            "Je suis Arthur, j'ai 24 ans et je suis jeune diplômé d'un lycée hôtelier et mon truc c'est les jeux de plateforme et les FPS. J'aime aussi bien les Open-Worlds mais avec parcimonie.", true//, $this->avatarUploader
        );
        $u2->setPassword($this->passwordEncoder->hashPassword($u2, "C'estMonMDP92"));
        
        $u3 = new User("Kurosaki", "Ichigo", "ichi.kuro@gmail.com", "ichikuro", "", 
            "Homme", new DateTime('2001-08-07'), false, 
            "Je m'appelle Ichigo, je suis un lycéen comme les autres ... Si ce n'est que j'ai la capacité de voir les fantômes, les esprits ... appelez ça comme vous voulez.\nUn jour, alors que ma famille était en danger, j'ai fait la rencontre d'une Shinigami nommée Rukia Kuchiki, après de grandes difficultés, elle m'a proposé de me préter ses pouvoirs pour protéger ma famille, j'ai accepté et je suis devenu à mon tour.\nJe vous passe les détails de la suite de l'histoire, mais on pourrait faire une histoire de plusieurs tomes de mangas haha !", true
        );
        ...
        $manager->persist($u3);
        $u4 = new User("La Cookie", "Lamia", "lamia.lacookie.etudiant@univ-lr.fr", "elDiableteDu17", 
            "MmEUwp567f89Bd", "Femme", new DateTime("2000-08-09"), false, 
            "Salut, je m'appelle Lamia, je suis une amie de Alexandre, alilizeph, avec qui j'ai passé une partie de mes études à L'Université de La Rochelle en licence informatique. Le développement Web c'est ma spécialité, actuellement, je suis en train de faire une licence professionnelle. \nQuand j'ai le temps, et j'en ai que trop peu à cause de la LP, je joue aux jeux vidéo et parfois, on essaye de se voir avec mon ami, mais c'est pas facile parce qu'il est actuellement dans sa campagne. (il fait pas d'effort aussi, l'Université encore moins !)", true//, $this->avatarUploader
        );
        $u4->setPassword($this->passwordEncoder->hashPassword($u4, "elDiableteDu17"));
        $manager->persist($u4);
        $u5 = new User("Doe", "John", "john.doe@gmail.com", "johndoe", "", 
            "Homme", new DateTime("1990-05-15"), false, 
            "Salut, je suis John Doe. J'adore les nouvelles technologies et l'informatique en général. J'aime résoudre des énigmes complexes et apprendre de nouvelles compétences en ligne. N'hésitez pas à me contacter si vous avez des questions sur la programmation ou si vous voulez discuter de sujets technologiques passionnants.", true//, $this->avatarUploader
        );
        $u5->setPassword($this->passwordEncoder->hashPassword($u5, "/pwdN578=PaSec&"));
        $manager->persist($u5);
        $u6 = new User("Smith", "Alice", "asmith@outlook.fr", "alsmith", "", 
            "Femme", new DateTime("1985-08-21"), false, 
            "Bonjour à tous, je suis Alice Smith. Je suis une passionnée de photographie et de voyage. J'adore capturer des moments spéciaux à travers mon objectif et partager mes aventures avec le monde. Si vous partagez ces intérêts ou si vous avez des conseils de voyage à partager, faites-moi signe !", true//, $this->avatarUploader
        );
        $u6->setPassword($this->passwordEncoder->hashPassword($u6, "pwdN6PaSec"));
        $manager->persist($u6);
        $u7 = new User("Brown", "Robert", "brob@orange.fr", "brob", "", 
            "Homme", new DateTime("1993-03-08"), false, 
        ...
        $manager->persist($u7);
        $u8 = new User("Johnson", "Emily", "emily.johnson@wanadoo.fr", "jemily", "", 
            "Femme", new DateTime("1988-05-14"), false, 
        ...
        $manager->persist($u8);
        $u9 = new User("Lee", "Michael", "lckael@gmail.com", "lkal", "", 
            "Homme", new DateTime("1991-11-30"), false, "", true//, $this->avatarUploader
        );
        $u9->setPassword($this->passwordEncoder->hashPassword($u9, "pwdN9PaSec"));
        $manager->persist($u9);
        $u10 = new User("Wang", "Lily", "lily.wang@univ-bdx.fr", "lilyWang", "", 
            "Femme", new DateTime("1995-02-18"), false, "", true//, $this->avatarUploader
        );
        $u10->setPassword($this->passwordEncoder->hashPassword($u10, "p(wdN108P<aSec"));
        $manager->persist($u10);
        $u11 = new User("Garcia", "Davis", "dave.garcia@gmail.com", "garvis", "", 
            "Homme", new DateTime("1982-06-06"), false, "", true
            //, $this->avatarUploader
        );
        $u11->setPassword($this->passwordEncoder->hashPassword($u6, "pw&dN11Pa4Se~c"));
        $manager->persist($u11);
        $u12 = new User("Kim", "Sophia", "ksophia@hotmail.com", "ksophia",  "", 
            "Ielle", new DateTime("1991-07-25"), false, "", "", true//, $this->avatarUploader
        );
         ...
         * OBJECTS VideoGame
         */
        $vg1 = new VideoGame("Pokémon: Légendes Arceus", $p1,  new DateTime("2022-01-28"), 
        ...
        $vg2 = new VideoGame("Pokémon: version Écarlate et Violet", $p1, new DateTime("2022-11-18"),
        ...
        $vg3 = new VideoGame("Hogwarts Legacy : L'Héritage de Poudlard", $p3, new DateTime("2023-02-10"),
        ...
        $vg4 = new VideoGame("Pokémon Rouge Feu/Vert Feuille", $p4, new DateTime("2004-01-29"),
        ...
        $vg5 = new VideoGame("Terra Nil", $p2, new DateTime("2023-03-28"),
            ...
            78, 24.99, true, "https://store.steampowered.com/app/1593030/Terra_Nil/", $l5
        );
        ...
        foreach(array($g6, $g12, $g8, $g4, $g7) as $g)
            $vg5->addGender($g);
        $manager->persist($vg5);


        $vg6 = new VideoGame("Farthest Frontier", $p2, new DateTime("2018-06-12"),
            ...
            47, 28.99, true,"https://store.steampowered.com/app/1044720/Farthest_Frontier/"
        );
        ...
        $vg7 = new VideoGame("Jurassic World Evolution", $p2, new DateTime("2022-08-09"),
            "Jurassic World Évolution est un jeu qui se déroule après les évènements du film Jurassic World où les gérants tentent de faire renaître de nouveau Jurassic Park. C'est donc à vous que revient cette tâche de faire grandir votre parc sur les îles des 5 morts ! C'est donc à vous de vous de subvenir aux besoins des visiteurs et de vos dinosaures avec les infrastructures à votre disposition et l'aide des différents services du parc.",
            "Jurassic World est un jeu de gestion exceptionnel ! Je ne compte plus les dizaines d'heures que j'ai passées à créer et recréer mon parc... Le jeu est bien équilibré et chaque île a ses spécificités : l'une est calme, une autre peut-être régulièrement prise dans une tempête tandis qu'une autre peut être endettée. A vous de trouver une solution aux problématiques et de faire des profits pour débloquer de nouvelles infrastructures.",
            82, 44.99, true,
        ...
        $vg8 = new VideoGame("Project Hospital", $p2, new DateTime("2018-09-28"),
        ...
        $vg9 = new VideoGame("Terraformers", $p2, new DateTime("2022-04-21"),
        ...
        $vg10 = new VideoGame("The Legend Of Zelda : Tears of the Kingdom", $p1, new DateTime("2023-05-12"),
        ...
        foreach(array($g1, $g2, $g5, $g12, $g3, $g7, $g4) as $g)
            $vg10->addGender($g);
        $manager->persist($vg10);


        $vg11 = new VideoGame("The Legend Of Zelda : Breath of the Wild", $p1, new DateTime("2017-03-03"),
        ...
        foreach(array($g1, $g2, $g5, $g12, $g3, $g7) as $g)
            $vg11->addGender($g);
        $manager->persist($vg11);


        $vg12 = new VideoGame("Dragon Quest XI : Les Combattants de la Destinée", $p1, new DateTime("2017-07-23"),
        ...
        $c1 = new Comments($u1, $vg1, "Un jeu sympathique",
            ...
            65, new DateTime("2022-07-03"));
        $manager->persist($c1);
        $c2 = new Comments($u6, $vg1, 'Super jeu !',
            "J'adore ce jeu, l'histoire est captivante et les graphismes sont superbes.",
            78, new DateTime("2023-01-15")
        );
        $manager->persist($c2);
        $c3 = new Comments($u6, $vg1, "Pas mal du tout",
            "Pokémon : Légendes Arceus est une expérience unique dans la série Pokémon.",
            71, new DateTime("2023-02-05")
        );
        $manager->persist($c3);
        ...
        $u->setUsername($s);
        $c10 = new Comments($u, $vg3, "Fantastique !",
            "Ce jeu est un rêve devenu réalité pour les fans d'Harry Potter.",
            90, new DateTime("2023-10-18")
        );
        $manager->persist($c10);
        $c11 = new Comments($u7, $vg3, "Bons souvenirs",
            "Retourner à Poudlard est toujours une expérience magique.",
            83, new DateTime("2023-10-02")
        );
        $manager->persist($c11);
        $c12 = new Comments($u6, $vg3, "A explorer",
            "L'univers d'Harry Potter est magnifiquement recréé.",
            74, new DateTime("2023-01-12")
        );
        $manager->persist($c12);
        $c13 = new Comments($u3, $vg3, "A explorer",
            "L'univers d'Harry Potter est magnifiquement recréé.",
            74, new DateTime("2023-12-01")
        );
        $manager->persist($c13);
        //  COMMENTS FOR Rouge Feu / Vert Feuille
        $c14 = new Comments($u1, $vg4, "Nostalgie",
            "Ces remakes me rappellent mon enfance.",
            67, new DateTime("2021-12-10")
        );
        $manager->persist($c14);
        ...
        $c15 = new Comments($u, $vg4, "Classique",
            "Pokémon Rouge Feu/Vert Feuille reste un classique intemporel.",
            76, new DateTime("2022-01-06")
        );
        $manager->persist($c15);
        $c16 = new Comments($u10, $vg4, "Un peu trop surestimé",
            ...
            74, new DateTime("2023-05-20")
        );
        $manager->persist($c16);
        //  COMMENTS FOR Terra Nil
        $c17 = new Comments($u7, $vg5, "Unique !",
            "Terra Nil propose une expérience de gestion écologique unique.",
            83, new DateTime("2022-07-08")
        );
        $manager->persist($c17);
        $c18 = new Comments($u5, $vg5, "Addictif",
            "Je ne peux pas m'arrêter de jouer à ce jeu.",
            87, new DateTime("2022-07-28")
        );
        $manager->persist($c18);
        // COMMENTS FOR Farthest Frontier
        $c19 = new Comments($u8, $vg6, "Du potentiel",
            "Le jeu a du potentiel, mais il a encore besoin de travail.",
            61, new DateTime("2022-09-10")
        );
        $manager->persist($c19);
        $c20 = new Comments($u12, $vg6, "Addictif",
            "Je ne peux pas m'arrêter de jouer à ce jeu.",
            59, new DateTime("2022-09-25")
        );
        $manager->persist($c20);
        ...
        $c24 = new Comments($u3, $vg8, "Décevant ...",
            ...
            46, new DateTime("2019-02-05")
        );
        $manager->persist($c24);
        //  COMMENTS FOR Terraformers
        $c25 = new Comments($u9, $vg9, "Exploration et aménagements",
            ...
            78, new DateTime("2022-03-10")
        );
        $manager->persist($c25);
        $c26 = new Comments($u3, $vg9, "Colonisation spatiale",
            "Excellent jeu rempli de bonnes idées, parfois un peu frustrant mais une fois pris en main, on passe un bon moment.",
            68, new DateTime("2022-03-14")
        );
        $manager->persist($c26);
        //  COMMENTS FOR Zelda TOTK 
        $c27 = new Comments($u6, $vg10, "Exploration et aménagements",
            "J'adore explorer Mars avec Terraformers ! Il faut bien réfléchir à comment agencer nos différentes colonies et trouver les bonnes ressources pou espérer rendre Mars viable pour l'Homme !",
            95, new DateTime("2023-04-15")
        );
        $manager->persist($c27);
        ...
        $u->setUsername($s);
        $u->setAvatarFile(null);
        $c28 = new Comments($u, $vg10, "Un jeu à couper le souffle !",
            "Les graphismes, l'histoire, les personnages, les combats, le gameplay ... tout est à couper le souffle !",
            98, new DateTime("2023-05-01")
        );
        $manager->persist($c28);
        //  COMMENTS FOR Zelda BOTW
        $c29 = new Comments($u2, $vg11, "Un chef-d'œuvre",
            "Breath of the Wild est un chef-d'œuvre du jeu vidéo.",
            93, new DateTime("2019-04-10")
        );
        $manager->persist($c29);
        $c30 = new Comments($u11, $vg11, "Un jeu à couper le souffle !",
            "Les graphismes, l'histoire, les personnages, les combats, le gameplay ... tout est à couper le souffle !",
            96, new DateTime("2020-04-25")
        );
        $manager->persist($c30);
        //  COMMENTS FOR DQ XI
        $c31 = new Comments($u9, $vg12, "Jeu chouette",
